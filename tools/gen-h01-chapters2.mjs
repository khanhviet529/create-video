...
.vv { position: absolute; left: ${RO.x}px; font-family: var(--font-value); font-size: 54px;
  ...
  js: [
    ...
    '}',
    "gsap.set(['#uLbl','#pLbl'], { opacity: 0 });",
    "gsap.set('.rule2', { opacity: 0 });",
    '',
    '/* beat 1-2 — hệ chạy tốt. Chỉ có ô của người dùng, và nó mang giá trị mới. */',
    "tl.to('#uLbl', { opacity: 1, duration: .5, ease: R }, 0.5);",
    "tl.to('.rule2', { opacity: .38, duration: .5, ease: R }, 0.5);",
    ...
    '',
    '/* beat 6 — hai lời giải thích bị loại. Trên màn hình KHÔNG gì đổi, và đó chính là lý do',
    '   đội đi tìm sai chỗ: loại một ứng viên không làm trạng thái đổi. Khoảng GIỮ có lý do. */',
    '/* beat 7 — "không node nào hỏng, cả hai đang trả lời đúng". Hai vạch sáng lên ĐỒNG THỜI',
    '   và GIỐNG NHAU: sự đồng thời và sự giống nhau chính là khẳng định. Vật đã có sẵn. */',
    "tl.to('.rule2', { opacity: 1, duration: .5, ease: R }, 11.0);",
    "tl.fromTo('.rule2', { scaleX: 1 }, { scaleX: 1.0, duration: .01 }, 11.0);",
  ].join('\n'),
});
...
build({
  ...
  js: [
    ...
    '',
    '/* beat 23 — "thêm replica MUA thêm khả năng đọc; thứ đem ĐỔI là read-your-writes".',
    '   Hai nửa của câu, hai vật đã có trên màn hình: tầm với của replica (mua được) rồi',
    '   khoảng chênh (đem đổi). Không thêm từ vựng, không thêm vật. */',
    "tl.fromTo('#bRep', { opacity: 1 }, { opacity: .45, duration: .35, ease: T }, 15.5);",
    "tl.to('#bRep', { opacity: 1, duration: .45, ease: R }, 15.9);",
    "tl.fromTo('#gapb', { opacity: .85 }, { opacity: .35, duration: .35, ease: T }, 16.6);",
    "tl.to('#gapb', { opacity: .85, duration: .45, ease: R }, 17.0);",
  ].join('\n'),
});
