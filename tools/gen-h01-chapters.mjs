...
build({
  ...
  js: [
    ...
    '/* "Commit KHÔNG TRẢ VỀ cho tới khi replica đã áp dụng xong" — một TRẠNG THÁI của dấu',
    '   commit, không phải một câu chú thích. Vòng mở ra và GIỮ: đang chờ. */',
    "tl.fromTo(rW, { opacity: 0, scale: .8 }, { opacity: .55, scale: 1, duration: .45, ease: R }, 15.7);",
    "tl.to('#bRep', { scaleX: " + ((X(500) - G.x0) / (X(499) - G.x0)).toFixed(4)
      + ", duration: 1.6, ease: T }, 16.2);",
    "tl.to('#cRep', { x: " + (X(500) - X(499)) + ", duration: 1.6, ease: T }, 16.2);",
    '/* beat 31 — điều kiện đạt: vòng đóng lại rồi nhả. Đây là chỗ commit TRẢ VỀ. */',
    "tl.to(rW, { opacity: 1, scale: 1, duration: .3, ease: R }, 18.0);",
